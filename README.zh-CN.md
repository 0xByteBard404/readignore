...
| **codex CLI** | **hard** | `.codex/hooks.json`，Claude-style 的 `PreToolUse` 钩子（bash，调 `readignore hook-check`）。与 Claude Code 同一套运行时拦截机制；首次运行需通过钩子信任提示。详见下方 [codex 平台说明](#codex-平台说明仅-bash-路径触发)。 | ✅ 已发布 |
...
| **kilo code** | config | `kilo.json` 里的 `permission.read` deny/allow glob；kilocode 加载配置时生效。 | ✅ 已发布 |

> ⚠️ **已知边界（Claude Code）：** `Read` 工具在某些配置下可绕过 `PreToolUse` 钩子 / `permissions.deny`（[anthropics/claude-code#37540](https://github.com/anthropics/claude-code/issues/37540)）。readignore 的钩子仍能显著抬高门槛；如需完整覆盖，请配合文件系统权限和/或密钥管理（零磁盘密钥）。
...
```
.claude/hooks/readignore.sh   (0755)  # 将 PreToolUse JSON 转发给 `readignore hook-check`
.claude/settings.json                 # 在 PreToolUse 上注册钩子
```

钩子在 `Read | Grep | Glob | Bash` 时触发，将 PreToolUse JSON 转发给
`readignore hook-check`（go-git 权威匹配），后者在 Go 中解析 `tool_input`，路径
命中时**在执行前拒绝**（输出 `permissionDecision: "deny"` JSON）。
Claude Code 的 settings 监听器会实时感知这一变更——**无需重启**。

**改 `.readignore` 立即生效**——钩子每次调用都通过 `readignore hook-check` 重新读
...
```
.codex/hooks/readignore.sh   (0755)  # 将 PreToolUse JSON 转发给 `readignore hook-check`
...
`readignore hook-check`（go-git 权威）并在工具执行前拒绝。与 Claude Code 一样，
...
- 让 codex 适配器与 Claude Code 适配器**对称**（共用同一套 bash 钩子，都调 `readignore hook-check`）；
...
适配器（opencode）。所有钩子都经由 `readignore` 二进制的匹配引擎（bash 钩子调
`readignore hook-check`，pi 扩展调 `readignore match`），因此**改 `.readignore`
立即生效**——无需重新 install。可通过 npm、`curl | sh` 或 Homebrew 安装。Cursor 和
kilo code 适配器在路线图上。
