...
```
.readignore  ──►  readignore CLI  ──►  各智能体的原生防御
(gitignore       (解析 + 适配)        • Claude Code: PreToolUse 钩子（hard 硬拦）
 语法)                                • codex CLI:   PreToolUse 钩子（hard，Claude-style）
                                      • pi:          覆写 `read` 工具的 TS 扩展（hard 硬拦）
                                      • opencode:    permissions deny 配置
                                      • Cursor:      路线图
```
...
| **codex CLI** | **hard** | `.codex/hooks.json`，Claude-style 的 `PreToolUse` 钩子（bash+python）。与 Claude Code 同一套运行时拦截机制；首次运行需通过钩子信任提示。详见下方 [codex 平台说明](#codex-平台说明仅-bash-路径触发)。 | ✅ 已发布 |
| **pi** | **hard** | `.pi/extensions/readignore.ts` TypeScript 扩展**覆写**内置 `read` 工具——匹配的路径在读取前返回 `Access denied`。启动时自动加载。 | ✅ 已发布 |
...
```
.claude/hooks/readignore.sh   (0755)  # 从 tool_input JSON 中提取目标路径
.claude/hooks/readignore.py   (0644)  # gitignore 匹配引擎（仅用标准库）
.claude/settings.json                 # 在 PreToolUse 上注册钩子
```

钩子在 `Read | Grep | Glob | Bash` 时触发，当目标路径匹配 `.readignore`
**在执行前拒绝**。Claude Code 的 settings 监听器会实时感知这一变更——**无需重启**。
...
在 `.codex/` 下生成三个文件，与 Claude Code 布局一致：

```
.codex/hooks/readignore.sh   (0755)  # 从 tool_input JSON 中提取目标路径
.codex/hooks/readignore.py   (0644)  # gitignore 匹配引擎（仅用标准库）
.codex/hooks.json                    # 注册钩子（Claude-style PreToolUse）
```

codex 的钩子协议是
[Claude-style](https://github.com/openai/codex)
（`PreToolUse` + `permissionDecision: "deny"`），因此同一套 bash+python 匹配引擎会
运行，并在工具执行前拒绝。
...
- 让 codex 适配器与 Claude Code 适配器**对称**（共用同一套 bash+python 钩子引擎）；
...
该扩展注册一个名为 `read` 的工具——与 pi 内置工具同名——从而**覆写**它：匹配的路径在
文件被读取前返回 `Access denied`；其余情况委托给正常读取。匹配器是手写的 gitignore
（零 npm 依赖）。不引入任何 pi 类型，因此文件可独立通过类型检查。
...
v0.2.0 —— 三个 **hard** 适配器（Claude Code、codex CLI、pi）+ 一个 **config**
适配器（opencode）。解析器、适配器注册表与 CLI 已稳定。Cursor 和 kilo code 适配器在
路线图上。
